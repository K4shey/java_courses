...
//        }
        ...
        int secondOccurrence = string.lastIndexOf("\t");
        try {
            return string.substring(firstOccurrence + 1, secondOccurrence);
        } catch (StringIndexOutOfBoundsException e) {
            if (threadName.equals(FIRST_THREAD_NAME)) {
                throw new StringForFirstThreadTooShortException(e.toString(), e);
            } else if (threadName.equals(SECOND_THREAD_NAME)) {
                throw new StringForSecondThreadTooShortException(e.toString(), e);
            } else {
